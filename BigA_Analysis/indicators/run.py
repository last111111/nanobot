...
from indicators.data_fetcher import get_price
from indicators.ta_functions import INDICATOR_REGISTRY, MACD, KDJ, RSI, BBANDS, ATR, OBV
...
def _unpack_tuple(name, result):
    ...
    if keys and len(keys) == len(result):
        return {f'{name}_{k}': _last_valid(v) for k, v in zip(keys, result)}
    else:
